  ...
  <view class="merchant-page">
    <view class="action-bar">
      ...
      <u-button
        ...
        icon="plus"
        text="加商品"
        customStyle="width: 180rpx; height: 64rpx; border-radius: 32rpx;"
    ...
    <view class="list-container">
      ...
      >
        ...
        <view v-else class="card-list">
          <!-- 展示模拟数据或实际数据 -->
          <view v-for="item in (data.length > 0 ? data : mockGoods)" :key="item._id" class="card goods-card" @click="navTo('/pages/merchant/goods/edit?id='+item._id)">
            ...
            <view class="goods-info">
              ...
              <view class="stock-row">
                库存: <text :class="{warn: item.stock < 10}">{{ item.stock }}</text> {{ item.unit_small.name }}
              </view>
              <view class="price-row">
                <text class="price">¥{{ (item.unit_small.price / 100).toFixed(2) }}/{{ item.unit_small.name }}</text>
...
const mockGoods = [
  { _id: 'm1', name: '农夫山泉 550ml', stock: 120, is_on_sale: true, unit_small: { name: '瓶', price: 100 }, img_url: '' },
  { _id: 'm2', name: '康师傅红烧牛肉面', stock: 5, is_on_sale: true, unit_small: { name: '包', price: 300 }, img_url: '' },
  { _id: 'm3', name: '雪碧 330ml', stock: 45, is_on_sale: true, unit_small: { name: '罐', price: 200 }, img_url: '' },
  { _id: 'm4', name: '金龙鱼大米 5kg', stock: 20, is_on_sale: true, unit_small: { name: '袋', price: 4500 }, img_url: '' },
  { _id: 'm5', name: '海天酱油 500ml', stock: 8, is_on_sale: false, unit_small: { name: '瓶', price: 800 }, img_url: '' }
]
...
.card {
  ...
  .goods-info {
    ...
    .price-row {
      ...
      justify-content: space-between;
      align-items: center;
      .price { color: #ff6b00; font-size: 32rpx; font-weight: bold; }
    }
  }
}
</style>
