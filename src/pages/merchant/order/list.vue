<template>
  <view class="merchant-page">
    <view class="tabs-box">
      <u-tabs :list="tabList" @click="tabClick" :current="currentTab" lineColor="#1890ff" activeStyle="color: #1890ff; font-weight: bold;"></u-tabs>
    </view>
    ...
    <view class="list-container">
      ...
      >
        ...
        <view v-else class="card-list">
          <view v-for="order in (data.length > 0 ? data : mockOrders)" :key="order._id" class="card order-card">
            ...
            <view class="card-body">
              ...
              <view class="items">
                <view v-for="(item, idx) in order.items" :key="idx" class="item-line">
                  {{ item.name }} x{{ item.countSmall || item.count }}{{ item.unitSmallName || item.unit }}
                </view>
              </view>
            </view>
            ...
            <view class="card-footer">
              ...
              <view class="right">
                ...
                <view class="btns">
                  <u-button v-if="order.status === 'pending'" type="primary" size="mini" text="确认接单" @click="updateStatus(order._id, 'confirmed')"></u-button>
                  <u-button v-if="order.status === 'confirmed'" type="success" size="mini" text="确认送达" @click="updateStatus(order._id, 'completed')"></u-button>
                </view>
              </view>
            </view>
          </view>
...
const mockOrders = [
  { _id: 'o1', order_no: 'ORD20250101001', status: 'pending', customer_name: '李老板', create_time: Date.now() - 3600000, total_amount: 5400, items: [{name: '农夫山泉', count: 2, unit: '箱'}] },
  { _id: 'o2', order_no: 'ORD20250101002', status: 'confirmed', customer_name: '张小店', create_time: Date.now() - 7200000, total_amount: 12000, items: [{name: '康师傅方便面', count: 2, unit: '箱'}] },
  { _id: 'o3', order_no: 'ORD20250101003', status: 'completed', customer_name: '王大妈', create_time: Date.now() - 86400000, total_amount: 3500, items: [{name: '雪碧', count: 1, unit: '箱'}] },
  { _id: 'o4', order_no: 'ORD20250101004', status: 'pending', customer_name: '赵记批发', create_time: Date.now() - 1800000, total_amount: 8500, items: [{name: '金龙鱼油', count: 1, unit: '桶'}] }
]
...
const getStatusText = (status: string) => {
  const map: any = { 'pending': '待确认', 'confirmed': '待发货', 'completed': '已完成', 'cancelled': '已取消' }
...
.card {
  ...
  .card-body {
    .customer { font-size: 30rpx; font-weight: bold; color: #333; margin-bottom: 12rpx; }
    .items {
      .item-line { font-size: 26rpx; color: #666; margin-bottom: 4rpx; }
    }
  }
  ...
  .card-footer {
    ...
    .right {
      ...
      align-items: center;
      gap: 20rpx;
      .total { font-size: 32rpx; color: #ff6b00; font-weight: bold; }
