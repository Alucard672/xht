...
| -------- | -------- | ------------------- | --------------- | ---------------------------------------- |
| **商家** | Merchant | 商家账号 (uni-id)   | 有效期制 (付费) | 商品管理、客户管理、订单处理、店铺二维码 |
| **客户** | Customer | 客户账号 (微信授权) | 免费            | 浏览店铺、下单、查看订单和欠款           |

> **注意：** SaaS服务商管理端已独立运行，通过外部接口对接，本项目不包含管理端代码。
...
| `status`          | Int       | 0:待接单, 1:已完成, -1:已取消                       |
| `payment_method`  | String    | `credit` (赊账), `wechat` (在线支付)                |
| `items`           | Array     | 商品快照数组                                        |
| `total_fee`       | Int       | 订单总金额 (分)                                     |
| `order_type`      | String    | **订单类型**: `customer`(客户下单), `agent`(代下单) |
| `created_by`      | String    | 创建人 UID                                          |
| `agent_tenant_id` | String    | **代下单商家ID** (仅代下单时填)                     |
| `created_at`      | Timestamp | 创建时间                                            |
...
└── agent (代下单)
    └── created_by: merchant
    └── agent_tenant_id: 商家ID
    └── 统计: 计入销售额，关联到客户
```

### 4.5 赊账记录表 (`wh_debt_logs`)
...
> **注意：** 记一笔/还一笔功能为次要业务，暂不考虑实现。

### 4.6 商家店铺二维码表 (`wh_shop_codes`)
...
### 4.7 客户关联商家表 (`wh_customer_tenants`)
...
2. **Price & Stock Logic:**
   ...
   - **Multi-Unit Cart:** Support simultaneous selection of big and small units.
     - Example: User can select `[2]箱[5]瓶` in cart.
     - Formula: `Total Price = (count_big * rate + count_small) * price_small`.

3. **Order Flow:**
   - Simple flow: Customer places order -> Merchant accepts -> Order completed.
   - No shipping confirmation step required.

4. **Offline-Friendly UI:**
