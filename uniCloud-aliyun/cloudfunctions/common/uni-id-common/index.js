'use strict'
var e,
  n = (e = require('crypto')) && 'object' == typeof e && 'default' in e ? e.default : e
const i = {
...
function c(e) {
  const { errCode: n, errMsgValue: i } = e
  ;((e.errMsg = this._t(n, i)), n in s && (e.code = s[n]), delete e.errMsgValue)
}
function a(e) {
  return (
    'object' === ((t = e), Object.prototype.toString.call(t).slice(8, -1).toLowerCase()) &&
    e.errCode &&
    ((n = e.errCode), Object.values(i).includes(n)) &&
    !!e.errCode
  )
  var n, t
}
...
class m {
  ...
  checkConfig() {
    const { tokenExpiresIn: e, tokenExpiresThreshold: n } = this.config
    if (n >= e)
  ...
  async getUserRecord() {
    ...
    if (((this.userRecord = e.data[0]), !this.userRecord)) throw { errCode: i.ACCOUNT_NOT_EXISTS }
    switch (this.userRecord.status) {
      case void 0:
      case 0:
        break
      case 1:
        throw { errCode: i.ACCOUNT_BANNED }
      case 2:
        throw { errCode: i.ACCOUNT_AUDITING }
      case 3:
        throw { errCode: i.ACCOUNT_AUDIT_FAILED }
      case 4:
        throw { errCode: i.ACCOUNT_CLOSED }
    }
  ...
  async getUserPermission() {
    ...
    const n = await T.where({ role_id: I.in(e) }).get(),
      i =
        ((t = n.data.reduce((e, n) => (n.permission && e.push(...n.permission), e), [])),
        Array.from(new Set(t)))
    var t
    return ((this.userPermission = { role: e, permission: i }), this.userPermission)
  }
  async _createToken({ uid: e, role: n, permission: t } = {}) {
    if (!n || !t) {
      const e = await this.getUserPermission()
      ;((n = e.role), (t = e.permission))
    }
    let r = { uid: e, role: n, permission: t }
    if (this.uniId.interceptorMap.has('customToken')) {
      ...
      r = await i({ uid: e, role: n, permission: t })
    }
    const o = Date.now(),
      { tokenSecret: s, tokenExpiresIn: c, maxTokenLength: a = 10 } = this.config,
      u = g({ ...r, uniIdVersion: '1.0.19' }, s, { expiresIn: c }),
  ...
  async createToken({ uid: e, role: n, permission: t } = {}) {
    if (!e) throw { errCode: i.PARAM_REQUIRED, errMsgValue: { param: 'uid' } }
    this.uid = e
  ...
  async refreshToken({ token: e } = {}) {
    if (!e) throw { errCode: i.PARAM_REQUIRED, errMsgValue: { param: 'token' } }
    this.oldToken = e
    const n = this._checkToken(e)
    ;((this.uid = n.uid), (this.oldTokenPayload = n))
    const { uid: t } = n,
      { role: r, permission: o } = await this.getUserPermission(),
      { token: s, tokenExpired: c } = await this._createToken({ uid: t, role: r, permission: o })
    return { errCode: 0, token: s, tokenExpired: c }
  }
  _checkToken(e) {
    const { tokenSecret: n } = this.config
    let t
    try {
      t = k(e, n)
    } catch (e) {
      if ('TokenExpiredError' === e.name) throw { errCode: i.TOKEN_EXPIRED }
      throw { errCode: i.CHECK_TOKEN_FAILED }
    }
    return t
  }
  async checkToken(e, { autoRefresh: n = !0 } = {}) {
    if (!e) throw { errCode: i.CHECK_TOKEN_FAILED }
    this.oldToken = e
    const t = this._checkToken(e)
    ;((this.uid = t.uid), (this.oldTokenPayload = t))
    const { tokenExpiresThreshold: r } = this.config,
      { uid: o, role: s, permission: c } = t,
      a = { role: s, permission: c }
    if (!s && !c) {
      const { role: e, permission: n } = await this.getUserPermission()
      ;((a.role = e), (a.permission = n))
    }
    if (!r || !n) {
      const e = { code: 0, errCode: 0, ...t, ...a }
      return (delete e.uniIdVersion, e)
    }
    const u = Date.now()
    let d = {}
    1e3 * t.exp - u < 1e3 * r && (d = await this._createToken({ uid: o }))
    const l = { code: 0, errCode: 0, ...t, ...a, ...d }
    return (delete l.uniIdVersion, l)
  }
}
var E = Object.freeze({
  __proto__: null,
  checkToken: async function (e, { autoRefresh: n = !0 } = {}) {
    return new m({ uniId: this }).checkToken(e, { autoRefresh: n })
  },
  createToken: async function ({ uid: e, role: n, permission: i } = {}) {
    return new m({ uniId: this }).createToken({ uid: e, role: n, permission: i })
  },
...
class x {
  constructor({ context: e, clientInfo: n, config: i } = {}) {
    ;((this._clientInfo = e
      ? (function (e) {
          return {
            appId: e.APPID,
